...
const db = require('./db'),
  fakeDbImpl = {
    raw: jest.fn((sql, args) => Promise.resolve({}))
  };
...
describe('db', () => {
  ...
  describe('deleteItem', () => {
    test('it calls raw once and pass the parameter as the second arg to raw', () => {
      const uri = 'http://localhost.nymag.com/_components/layout-video/instances/original-vi';
  ...
  describe('insertItem', () => {
    test('it calls raw once and pass the reference and the data to the raw function', () => {
      const reference = 'http://localhost.nymag.com/_components/layout-video/instances/original-vi',
        data = {
          at: 123456789,
          publish: 'http://localhost.nymag.com/_components/layout-video/instances/original-vi'
        };

      return db.insertItem(reference, data).then(() => {
        expect(db.db.raw.mock.calls.length).toBe(1);
        const callParams = db.db.raw.mock.calls[0][1];

        expect(callParams[0]).toBe(reference);
  ...
  describe('selectPublishableItems', () => {
    ...
    test('it throws an error if bad uri is passed', () => {
      ...
      expect(db.selectPublishableItems.bind(null, currentTime)).toThrow(`Must be a number, current value: ${currentTime}`);
    });
  });
